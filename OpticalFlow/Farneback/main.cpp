...
int main(int, char**)
{
    VideoCapture cap;
	cap.open(0);
	//cap.open("E://OpenCV_Projects//OpticalFlow//openCloseLight2.avi");
	//cap.open("E://OpenCV_Projects//OpticalFlow//test2.avi");
        ...
        if( prevgray.data )
        {
			...
			for (int y = 0; y < cflow.rows; y += 10 )
			{
				for (int x = 0; x < cflow.cols; x += 10)
				{
					...
					line(cflow, Point(x, y), Point(cvRound(x+fxy.x), cvRound(y+fxy.y)), CV_RGB(176,224,230));
					circle(cflow, Point(x,y), 2, CV_RGB(242, 156, 177), -1);
				}
			}
			if(cflow.data)imshow("FLOW", cflow);
        }
        if(waitKey(10)>=0)
            break;
